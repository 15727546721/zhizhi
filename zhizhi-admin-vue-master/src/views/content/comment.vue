  ...
  <div class="app-container">
    ...
    <el-card shadow="never" class="table-container">
      ...
      >
        ...
        <el-table-column width="100" align="center" label="回复数">
          <template #default="scope">
            <el-tag :type="scope.row.replyCount > 0 ? 'primary' : 'info'" size="small">
        ...
        >
          <template #default="scope">
            <el-button
              ...
              size="small"
              :disabled="!scope.row.replyCount || scope.row.replyCount === 0"
            >
              <el-icon><View /></el-icon>回复({{ scope.row.replyCount || 0 }})
            </el-button>
            <el-button
              link
              type="danger"
...
function handleViewReplies(row: CommentDTO) {
  if (!row.replyCount || row.replyCount === 0) {
  ...
  getReplies(row.id, { pageNo: 1, pageSize: 50 })
    .then(({ data }) => {
      // 后端返回 ResponseEntity，数据在 data 字段中
      repliesList.value = data.data || data || [];
    })
    .catch((error) => {
