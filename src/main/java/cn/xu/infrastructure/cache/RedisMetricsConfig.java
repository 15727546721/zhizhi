...
public class RedisMetricsConfig {
    ...
    private double getActiveConnections(RedisConnectionFactory factory) {
        try (RedisConnection connection = factory.getConnection()) {
            ...
            log.debug("Redis活跃连接数: {}", value);
            return value;
        } catch (Exception e) {
            log.error(LogConstants.REDIS_METRICS_COLLECT_FAILED, "连接池", e.getMessage());
            return -1;
        }
    }
    ...
    private double getUsedMemory(RedisConnectionFactory factory) {
        try (RedisConnection connection = factory.getConnection()) {
            ...
            log.debug("Redis内存使用: {} bytes", value);
            return value;
        } catch (Exception e) {
    ...
    private double getTotalKeys(RedisConnectionFactory factory) {
        try (RedisConnection connection = factory.getConnection()) {
            ...
            log.debug("Redis键总数: {}", value);
            return value;
        } catch (Exception e) {
            log.error(LogConstants.REDIS_METRICS_COLLECT_FAILED, "键统计", e.getMessage());
            return -1;
        }
    }
} 