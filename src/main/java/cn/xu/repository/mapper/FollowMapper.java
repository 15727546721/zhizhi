...
public interface FollowMapper {
     ...
     * 查询关注列表（别名方法，简化调用）
     */
    default List<Follow> findFollowingList(Long followerId, int offset, int size) {
        return listFollowingByPage(followerId, offset, size);
    }
    
    /**
     * 查询粉丝列表（别名方法，简化调用）
     */
    default List<Follow> findFollowersList(Long followedId, int offset, int size) {
        return listFollowersByPage(followedId, offset, size);
    }
