...
Post模块是知之社区系统的核心功能模块之一，支持多种类型的内容发布，包括文章、普通帖子、讨论帖、问答帖等。该模块遵循领域驱动设计（DDD）原则，采用清晰的分层架构，实现了完整的帖子生命周期管理。
...
1. **ARTICLE（文章）**：用于发布长篇内容，适合技术文章、教程等
2. **POST（普通帖子）**：用于一般性内容发布，适合日常分享、动态等
3. **DISCUSSION（讨论帖）**：用于发起讨论话题，适合社区讨论
4. **QUESTION（问答帖）**：用于提问和回答，支持采纳最佳回答功能
